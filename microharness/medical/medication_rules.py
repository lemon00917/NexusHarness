...
DEFAULT_FIELD_ROLES: dict[str, tuple[str, ...]] = {
    "record_id": ("medPrescNo", "处方号", "医嘱号", "记录ID"),
...
def parse_medication_records(
    ...
    for binding in bindings or []:
        ...
        grouped.setdefault(prefix, []).append({"label": label, "eng": eng, "value": value})
    ...
    for prefix, fields in grouped.items():
        ...
        if values.get("entity") or values.get("category"):
            records.append(MedicationRecord(prefix=prefix, values=values, raw_fields=fields))
    return records
...
def judge_medication_condition(
    ...
    *,
    ...
    candidates = [record for record in records if _entity_matches(entity, record)]
    target = entity or "目标用药"
    ...
    if time_window and time_window.required and not time_window.resolved:
        details = [_candidate_detail(record, predicate, None, "", None, time_window, "缺少事件时间锚点，无法计算目标时间窗") for record in candidates]
        return {
            "applicable": True, "matched": False, "status": "UNKNOWN",
    ...
    if matched_records:
        names = "；".join(
            f"{record.prefix} 医嘱项={record.get('entity')}，开立时间={record.get('ordered_at') or '未取得'}，"
            f"医嘱状态={_display_status(record)}"
    ...
    detail_reason = "；".join(
        f"{item.get('记录')} 医嘱项={item.get('医嘱项')}，{item.get('时间判断')}，{item.get('状态判断')}"
